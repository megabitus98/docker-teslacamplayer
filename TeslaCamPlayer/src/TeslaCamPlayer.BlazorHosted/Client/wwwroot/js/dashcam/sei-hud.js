...
(function () {
  ...
  function normalizeTelemetry(raw, opts) {
    const speedMps = pickNumber(raw, ["vehicleSpeedMps", "vehicle_speed_mps"], 0);
    ...
    const gearState = pickString(raw, ["gearState", "gear_state"], "");

    return {
      speed: opts.useMph ? speedMph : speedMph * 1.609,
      steerDeg: pickNumber(raw, ["steeringWheelAngle", "steering_wheel_angle"], 0),
      left: pickBool(raw, ["blinkerOnLeft", "blinker_on_left"]),
      right: pickBool(raw, ["blinkerOnRight", "blinker_on_right"]),
      brake: pickBool(raw, ["brakeApplied", "brake_applied"]),
      throttlePct: clamp(throttleRaw, 0, 100),
      autopilotState: pickString(raw, ["autopilotState", "autopilot_state"]),
      gear: gearMapping[gearState] || "—"
    };
  }
...
`;
  ...
  function mount({ videoEl, getTelemetry }) {
    ...
    const stop = rafLoop(() => {
      ...
      const isAP = t.autopilotState && t.autopilotState !== "NONE" && t.autopilotState !== "OFF";
      hud.apStatus.classList.toggle("active", isAP);
      if (isAP) {
        if (t.autopilotState === "SELF_DRIVING") hud.apStatus.textContent = "Self-Driving";
        else if (t.autopilotState === "AUTOSTEER") hud.apStatus.textContent = "Autosteer";
        else if (t.autopilotState === "TACC") hud.apStatus.textContent = "TACC";
      }
    });
